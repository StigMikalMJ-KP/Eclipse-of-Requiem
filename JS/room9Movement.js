...
if (!player) {
    ...
    // Append to room-stage so it stays correctly positioned with the background
    const stage = document.getElementById("room-stage") || document.getElementById("container");
...
const PLAYER_SPRITES = {
    ...
    left: {
        ...
        walk: ["../Assets/Karakter/Side/Left/Left-Base-Walk-1.png"]
    },
    right: {
...
let enabled_keys = { LEFT: true, UP: true, RIGHT: true, DOWN: true };
...
function applySpawnPosition() {
    ...
    try {
        ...
        if (spawn.room !== currentRoom) return;

        const mapRect = map.getBoundingClientRect();
        xPos = (spawn.x / 10) * mapRect.width;
...
function handle_input() {
    // --- CRITICAL FIX: Stop normal walking when mirror is active ---
    if (window.mirrorActive) return;

    enabled_keys = { LEFT: true, UP: true, RIGHT: true, DOWN: true };

    if (map) check_bounds(map);

    if ((keys_pressed["KeyA"] || keys_pressed["ArrowLeft"]) && enabled_keys["LEFT"]) {
        speed_x = clamp(speed_x + acceleration, min_speed, max_speed);
        speed_y = clamp(speed_y - acceleration, min_speed, max_speed);
        const nextX = xPos - speed_x;
        playerFacing = "left";
        if (!collidesWithHitbox(-speed_x, 0)) xPos = nextX;
    }
    ...
    if ((keys_pressed["KeyW"] || keys_pressed["ArrowUp"]) && enabled_keys["UP"]) {
        ...
        playerFacing = "back";
        if (!collidesWithHitbox(0, -speed_y)) yPos = nextY;
    }
    ...
    const isMoving = Boolean(
        keys_pressed["KeyA"] || keys_pressed["ArrowLeft"] ||
        keys_pressed["KeyD"] || keys_pressed["ArrowRight"] ||
        keys_pressed["KeyW"] || keys_pressed["ArrowUp"] ||
        keys_pressed["KeyS"] || keys_pressed["ArrowDown"]
    );
...
function setPlayerSprite(spritePath) {
    if (!spritePath || spritePath === currentSpritePath) return;
...
function updatePlayerSprite(isMoving) {
    ...
    const scaleX = usesMirroredRight ? -1 : 1;
...
function checkTriggerZones() {
    // --- Do not trigger map teleports while inspecting the mirror ---
    if (!map || isHandlingTrigger || window.mirrorActive) return;

    let activeHitboxes = getActiveHitboxes().filter(isTriggerZone);
    ...
    for (const hitbox of activeHitboxes) {
        ...
        if (intersects) {
            ...
            setTimeout(() => { isHandlingTrigger = false; }, 200);
            return;
        }
    }
}
...
function check_bounds(obj) {
    ...
    if (!(plr.left - margin >= obj_rect.left)) enabled_keys.LEFT = false;
    if (plr.top - margin <= obj_rect.top) enabled_keys.UP = false;
    if (plr.right + margin >= obj_rect.right) enabled_keys.RIGHT = false;
    if (plr.bottom + margin >= obj_rect.bottom) enabled_keys.DOWN = false;
}
...
function fadeInFromBlackIfNeeded() {
    ...
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            overlay.style.opacity = "0";
            setTimeout(() => overlay.remove(), TRANSITION_DURATION_MS);
        });
    });
}
...
function handleTrigger(hitbox) {
    ...
    if (trigger.type !== "teleport" || !trigger.room) return;
